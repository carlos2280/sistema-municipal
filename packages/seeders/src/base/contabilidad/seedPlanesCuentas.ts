...
/**
 * Plan de cuentas contable base (ano 2025).
 * Idempotente: verifica si ya existen registros para ese ano antes de insertar.
 */
export async function seedPlanesCuentas(db: DbExecutor): Promise<void> {
  process.stdout.write("  seedPlanesCuentas: verificando catalogo existente...\n");
  ...
  if (existentes.length > 0) {
    process.stdout.write(`  seedPlanesCuentas: ya existen ${existentes.length} registros, saltando\n`);
    return;
  }
  ...
  const getSubgrupoId = (codigo: string): number => {
    ...
    if (!id) throw new Error(`Subgrupo ${codigo} no encontrado — ejecutar seedSubgrupos primero`);
    return id;
  };
  ...
  process.stdout.write("  seedPlanesCuentas: insertando cuentas nivel 1...\n");
  ...
  const cuentasNivel2 = [
    { anoContable: ANO, codigo: "1110101", nombre: "Caja Chica Municipal",        subgrupoId: getSubgrupoId("111"), parentId: idPorCodigo.get("11101"), codigoIni: "01", tipoCuentaId: 5 },
    { anoContable: ANO, codigo: "1110102", nombre: "Caja Recaudadora",             subgrupoId: getSubgrupoId("111"), parentId: idPorCodigo.get("11101"), codigoIni: "02", tipoCuentaId: 5 },
    { anoContable: ANO, codigo: "1110201", nombre: "Cuenta Corriente Principal",   subgrupoId: getSubgrupoId("111"), parentId: idPorCodigo.get("11102"), codigoIni: "01", tipoCuentaId: 5 },
  ];

  process.stdout.write("  seedPlanesCuentas: insertando cuentas nivel 2...\n");
  await db.insert(planesCuentas).values(cuentasNivel2);

  process.stdout.write(`  seedPlanesCuentas: ${cuentasNivel1.length + cuentasNivel2.length} cuentas insertadas\n`);
}
