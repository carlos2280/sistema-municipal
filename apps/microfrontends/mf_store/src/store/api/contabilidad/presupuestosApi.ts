...
export const presupuestosApi = baseApi.injectEndpoints({
	endpoints: (builder) => ({
		...
		// ── Presupuestos ──
		listarPresupuestos: builder.query<PresupuestoResumen[], number | undefined>(
			{
				query: (ano) =>
					ano
						? `contabilidad/presupuestos?ano=${ano}`
		...
		crearPresupuesto: builder.mutation<
			PresupuestoResumen,
		...
		actualizarPresupuesto: builder.mutation<
			PresupuestoResumen,
		...
		eliminarLinea: builder.mutation<
			void,
			{ presupuestoId: number; detalleId: number }
		>({
