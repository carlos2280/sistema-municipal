...
vi.mock("otplib", () => ({
  authenticator: {
    ...
    keyuri: vi.fn(() => "otpauth://totp/Sistema%20Municipal:user@muni.cl?secret=JBSWY3DPEHPK3PXP"),
    verify: vi.fn(),
  },
}));
...
describe("mfa.service", () => {
  ...
  describe("setupMfa()", () => {
    ...
    it("debería lanzar 409 si MFA ya está activo y verificado", async () => {
      ...
      await expect(
        setupMfa(db as never, 1, "user@muni.cl"),
      ).rejects.toThrow("MFA ya está activo en esta cuenta.");
    });
  ...
  describe("verifyMfaCode()", () => {
    ...
    it("debería lanzar 401 con código TOTP inválido", async () => {
      ...
      await expect(
        verifyMfaCode(db as never, 1, "000000"),
    ...
    it("debería lanzar error si MFA no está habilitado", async () => {
      ...
      await expect(
        verifyMfaCode(db as never, 1, "123456"),
      ).rejects.toThrow("MFA no está habilitado");
    });
  });
