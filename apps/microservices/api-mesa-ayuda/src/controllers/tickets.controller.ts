...
export async function listar(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const result = await ticketsService.listarTickets(getDB(), tenantId, filters);
    res.json(result);
  } catch (err) {
    next(err);
  }
...
export async function crear(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const ticket = await ticketsService.crearTicket(getDB(), user.tenantId, req.body, user);
...
export async function actualizar(req: Request, res: Response, next: NextFunction) {
  try {
    const id = Number(req.params.id);
    const { tenantId } = getUserInfo(req);
    const ticket = await ticketsService.actualizarTicket(getDB(), tenantId, id, req.body);
    res.json(ticket);
  } catch (err) {
    next(err);
  }
...
export async function cambiarEstado(req: Request, res: Response, next: NextFunction) {
  try {
    const id = Number(req.params.id);
    const user = getUserInfo(req);
    const ticket = await ticketsService.cambiarEstado(getDB(), user.tenantId, id, req.body, user.id);
    res.json(ticket);
  } catch (err) {
    next(err);
  }
...
export async function asignar(req: Request, res: Response, next: NextFunction) {
  try {
    const id = Number(req.params.id);
    const { tenantId } = getUserInfo(req);
    const ticket = await ticketsService.asignarTicket(getDB(), tenantId, id, req.body);
    res.json(ticket);
  } catch (err) {
    next(err);
  }
