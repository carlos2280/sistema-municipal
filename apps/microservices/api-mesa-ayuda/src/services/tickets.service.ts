...
export async function listarTickets(
  ...
  if (filters.estado) {
    ...
    conditions.push(
      or(...estados.map((e) => eq(tickets.estado, e.trim()))) as ReturnType<
        typeof eq
      >,
    )
  }
...
export async function crearTicket(
  ...
  const fechaLimite = new Date(Date.now() + prioridad.slaHoras * 60 * 60 * 1000)
...
export async function obtenerStats(db: DbClient, tenantId: number) {
  ...
  const [estadoStats] = await db
    .select({
      ...
      enProgreso: count(
        sql`CASE WHEN ${tickets.estado} = 'en_progreso' THEN 1 END`,
      ),
      enEspera: count(
        sql`CASE WHEN ${tickets.estado} = 'en_espera' THEN 1 END`,
      ),
      resueltos: count(
        sql`CASE WHEN ${tickets.estado} = 'resuelto' THEN 1 END`,
      ),
