...
export const actualizarPlanesCuenta: RequestHandler = async (
  ...
  try {
    const updated = await pcService.actualizarPlanesCuenta(
      getDb(req),
      id,
      req.body,
    );
...
export const buscarCuentasPorPrefijo: RequestHandler = async (
  req,
  res,
  next,
) => {
...
export const verificarCodigoExiste: RequestHandler = async (req, res, next) => {
  ...
  if (!anoContable || !codigo) {
    return next(
      new AppError("Parámetros anoContable y codigo son requeridos", 400),
    );
  }
  ...
  try {
    const result = await pcService.verificarCodigoExiste(
      getDb(req),
      ano,
      String(codigo),
    );
