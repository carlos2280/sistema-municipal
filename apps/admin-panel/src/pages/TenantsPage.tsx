...
export default function TenantsPage() {
  ...
  return (
    <Box>
      ...
      {filtered?.length === 0 ? (
        <EmptyState icon={Building2} message="No se encontraron municipalidades" ctaLabel="Nueva Municipalidad" onCta={() => setDialogOpen(true)} />
      ) : (
        <Table>
          <TableHead>
            <TableRow>
          ...
          <TableBody>
            {filtered?.map((t) => (
              <TableRow key={t.id} hover sx={{ cursor: 'pointer' }} onClick={() => navigate(`/tenants/${t.id}`)}>
                ...
                <TableCell>{new Intl.DateTimeFormat('es-CL').format(new Date(t.createdAt))}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
      ...
      <CrearTenantDialog
        ...
        onClose={() => { setDialogOpen(false); setCreateError('') }}
        onChange={setForm}
