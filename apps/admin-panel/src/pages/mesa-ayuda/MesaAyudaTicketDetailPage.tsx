...
export default function MesaAyudaTicketDetailPage() {
  ...
  const {
    ...
    isLoading,
    error,
  } = useMesaAyudaTicketDetail(tenantSlug, id)
  ...
  if (error)
    return (
      <Alert severity="error">Error al cargar ticket: {error.message}</Alert>
    )
  ...
  function handleCambiarPrioridad(data: CambiarPrioridadInput) {
    cambiarPrioridad.mutate(
      ...
      {
        onSuccess: () => {
          setPrioridadOpen(false)
          onSuccess('Prioridad actualizada')
        },
      },
    )
  }
  function handleCambiarCategoria(data: CambiarCategoriaInput) {
    cambiarCategoria.mutate(
      { tenantSlug, ticketId: id, data },
      {
        onSuccess: () => {
  ...
  function handleAgregarComentario(data: AgregarComentarioInput) {
    agregarComentario.mutate(
      { tenantSlug, ticketId: id, data },
      { onSuccess: () => onSuccess('Comentario agregado') },
    )
  }

  const prioridadActual = prioridades.find(
    (p) => p.codigo === ticket.prioridadCodigo,
  )
      ...
      <Grid container spacing={3}>
        <Grid size={{ xs: 12, md: 8 }}>
          <TicketHeader
            ...
            onCambiarEstado={() => setEstadoOpen(true)}
            onAsignar={() => setAsignarOpen(true)}
            onCambiarPrioridad={() => setPrioridadOpen(true)}
            onCambiarCategoria={() => setCategoriaOpen(true)}
          />
          <Box sx={{ mb: 3 }}>
